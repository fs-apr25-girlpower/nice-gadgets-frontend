...
export const ProductCard = forwardRef<HTMLDivElement, Props>(
  ({ product, isLoading }, ref) => {
    const { currentLanguage } = useLanguage();
    const translations = productCardDictionary[currentLanguage];

    const hasDiscount =
      product?.year !== undefined ? product.year < 2020 : false;
...
ProductCard.displayName = 'ProductCard'; // forwardRef(...) створює анонімну обгортку  тому прописано явно для ESlint
